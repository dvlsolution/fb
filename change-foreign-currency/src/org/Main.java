...
public class Main
{
     ...
     */
    boolean canGetExactChange(int targetMoney, int[] denominations, Map<Integer, Boolean> memo) {
        if (memo.containsKey(targetMoney)) return memo.get(targetMoney);
        if (targetMoney == 0) return true;
        if (targetMoney < 0) return false;
        for (int i = 0; i < denominations.length; i++) {
            int reminder = targetMoney - denominations[i];
            if (canGetExactChange(reminder, denominations, memo)) {
                memo.put(reminder, true);
                return true;
            }
        }

        memo.put(targetMoney, false);
        return false;
    }
    ...
    boolean canGetExactChange(int targetMoney, int[] denominations) {
        ...
        return canGetExactChange(targetMoney, denominations, memo);
    }
