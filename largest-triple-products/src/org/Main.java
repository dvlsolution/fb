...
public class Main {
    ...
    int[] findMaxProduct(int[] arr) {
        ...
        // initialize the first 3 outputs
        ...
        // stack of prev max values
        PriorityQueue<Integer> maxPrevs = new PriorityQueue<>(new Comparator<Integer>() {
        ...
        for (int i = 3; i < arr.length; i++) {
            if (arr[i] > maxPrevs.peek()) {
                maxPrevs.remove();
                maxPrevs.add(arr[i]);

                // re-calc max product
                Integer[] tmp = maxPrevs.toArray(new Integer[0]);
