...
public class Main {
    ...
    /*
        ...
        2. Of the elements that were popped, find the one with the largest value (if there are multiple such elements,
        ...
        In the first iteration, the first 5 elements are popped off the queue, leaving just [5]. Of the popped elements, the largest one is the 4,
        which was at index 5 in the original array. The remaining elements are then decremented and added back onto the queue, whose contents are then [5, 0, 1, 1, 2].

        In the second iteration, all 5 elements are popped off the queue. The largest one is the 5, which was at index 6 in the original array.
        ...
        In the third iteration, all 4 elements are popped off the queue. The largest one is the 1, which had the initial value of 3 at index 4 in the original array.
        The remaining elements are added back onto the queue, whose contents are then [0, 0, 0].

        In the fourth iteration, all 3 elements are popped off the queue. Since they all have an equal value, we remove the one that was popped first,
        which had the initial value of 1 at index 1 in the original array. The remaining elements are added back onto the queue, whose contents are then [0, 0].
    ...
    int[] findPositions(int[] arr, int x) {
        ...
        if (arr.length == 0 || x == 0) return arr;

        List<Integer> result = new ArrayList<>();
        LinkedList<int[]> queue = new LinkedList<>();
        LinkedList<int[]> leftovers = new LinkedList<>();

        // convert input array into queue, first element is a value, the second is 1 based index
        for (int i = 0; i < arr.length; i++) {
            int[] element = new int[] {arr[i], i + 1};
        ...
        for (int i = 0; i < x; i++) {
            ...
            // remove max and put it into result ignoring zero values
            leftovers.remove(max);
            if (max[1] != 0) {
                result.add(max[1]);
            }

            // decrement the rest elements and put them back to queue
            while (!leftovers.isEmpty()) {
                int[] current = leftovers.removeFirst();
